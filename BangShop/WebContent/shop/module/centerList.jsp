...
<%
	...
	try {      
		...
		if (kind.equals("wc")) {
			kindKor = "여성의류";
		} else if (kind.equals("mc")) {
			kindKor = "남성의류";
		} else if (kind.equals("fs")) {
			kindKor = "패션잡화";
		} else if (kind.equals("ch")) {
			kindKor = "유아동/출산";
		} else if (kind.equals("sp")) {
			kindKor = "스포츠/레저";
		} else if (kind.equals("dg")) {
			kindKor = "디지털/가전";
		} else if (kind.equals("bk")) {
			kindKor = "도서/티켓";
		} else if (kind.equals("lv")) {
			kindKor = "생활/가구";
		} else if (kind.equals("st")) {
			kindKor = "스타굿즈";
		} else if (kind.equals("cr")) {
			kindKor = "차량";
		} else if (kind.equals("et")) {
			kindKor = "기타";
		}
		productList = productDao.getProductCategory(kindKor);
...
<body>
	...
	<nav class="w3-sidebar w3-bar-block w3-card" id="mySidebar">
		...
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=wc">여성의류</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=mc">남성의류</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=fs">패션잡화</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=ch">유아동/출산</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=sp">스포츠/레저</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=dg">디지털/가전</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=bk">도서/티켓</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=lv">생활/가구</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=st">스타굿즈</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=cr">차량</a>
		<a class="w3-bar-item w3-button" href="listMain.jsp?kind=et">기타</a>
	</nav>
	...
	<%
		...
		if (productList == null) {
	%>
		<h4 style="text-align: center; margin-top: 130px; margin-bottom: 130px;">
			<i class='far fa-grin-beam-sweat' style="margin-left: 10px; margin-right: 10px;"></i>
			등록된 상품이 없습니다.
		</h4>
